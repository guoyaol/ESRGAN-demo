...
var tvmjs__namespace = /*#__PURE__*/_interopNamespaceDefault(tvmjs);
...
class RealESRGANInstance {
  ...
  async #asyncInitTVM(wasmUrl, cacheUrl) {
    ...
    const tvm = await tvmjs__namespace.instantiate(
    ...
    // initialize WebGPU
    try {
      const output = await tvmjs__namespace.detectGPUDevice();
      if (output !== undefined) {
        var label = "WebGPU";
  ...
  async asyncInit() {
    if (this.pipeline !== undefined) return;
    await this.#asyncInitTVM("node_modules/real-esrgan/dist/real_esrgan_webgpu.wasm", "web-eargan-shards/");
